...
public class JellyBusinessTableService implements IJellyBusinessTableService {
	...
	public ResultData queryAll(Long companyId, String name, String remark) {
    	...
    	.like(!StringUtils.isEmpty(name), JellyBusinessTable::getName, name)
    	.like(!StringUtils.isEmpty(remark), JellyBusinessTable::getRemark, remark)
    	.orderByAsc(JellyBusinessTable::getCreateTime);
	...
	public ResultData insert(JellyBusinessTable businessTable) {
		...
		LambdaQueryWrapper<JellyBusinessTable> qw = new LambdaQueryWrapper<>();
		qw.eq(JellyBusinessTable::getName, businessTable.getName());
		...
		if (count == 0) {
			...
			if (datasourceUrl.contains("mysql")) {
				...
				sql.append(" ) ");
				sql.append(
						"ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_unicode_ci COMMENT = '");
	...
	public ResultData queryByTableIds(String tableIds) {
		LambdaQueryWrapper<JellyBusinessTable> qw = new LambdaQueryWrapper<>();
		qw.in(!StringUtils.isEmpty(tableIds), JellyBusinessTable::getId, Arrays.asList(tableIds.split(",")));
		List<JellyBusinessTable> list = businessTableMapper.selectList(qw);
		return ResultData.success(list);
	}
	...
	public ResultData queryByType(String type) {
		LambdaQueryWrapper<JellyBusinessTable> qw = new LambdaQueryWrapper<>();
		qw.like(JellyBusinessTable::getName, "_" + type + "_");
		List<JellyBusinessTable> tableList = businessTableMapper.selectList(qw);
	...
	public ResultData queryModel(Long companyId) {
		...
		List<Map<String, Object>> nodeslist = businessTableMapper.queryModel(companyId);
		List<Map<String, Object>> edgesList = new ArrayList<Map<String, Object>>();
		for (Map<String, Object> map : nodeslist) {
			List<Map<String, Object>> tableMapList = (List<Map<String, Object>>) map.get("authority");
			Map<String, Object> keyMap = new HashMap<String, Object>();
			keyMap.put("name", "id");
			keyMap.put("length", 20);
			keyMap.put("remark", "主键");
			keyMap.put("type", "bigint");
			List<Map<String, Object>> tempMap = new ArrayList<Map<String, Object>>();
			tempMap.add(keyMap);
			for (Map<String, Object> tableMap : tableMapList) {
				tempMap.add(tableMap);
				if (tableMap.get("target_table_id") != null) {
					Map<String, Object> edgesMap = new HashMap<String, Object>();
					edgesMap.put("source", "key" + tableMap.get("business_table_id"));
					edgesMap.put("target", "key" + tableMap.get("target_table_id"));
					edgesList.add(edgesMap);
				}
			}
			map.put("authority", tempMap);
			map.put("keyId", "key" + map.get("keyId"));
		}
		resMap.put("nodesList", nodeslist);
		resMap.put("edgesList", edgesList);
